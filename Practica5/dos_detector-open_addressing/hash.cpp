...
uniform()
{
    ...
    ret_v = rand() / (RAND_MAX + 1.0); //usamos randmax + 1 porque no queremos incluir el 1
    //
...
pick_at_random(std::uint64_t const& a, std::uint64_t const& b)
{
    ...
    //Hint: use uniform()
    double aux=uniform();
    ret_v =a + aux*(b-a);//cambio el intervalo 0-1 por el intervalo a-b y el numero aleatorio se adapta
    //
               ...
               std::uint64_t const& b)
{
    ...
    ret_v=static_cast<size_t>( ((a*key+b)% p)% m);
    //
    assert(ret_v<m);
    return ret_v;
}
...
QPHash::operator()(const uint64_t &k, size_t iter) const
{
    ...
    if(iter==0){
        ...
        ret_v=(static_cast<uint64_t>(universal_hash(k, m_, p_, a_, b_) + (iter * 0.5) + (iter*iter*0.5))) % m_;
        //funciona tambien usando static_cast<size_t>
    }
    //
    return ret_v;
}
...
RPHash::operator()(const uint64_t &k, size_t iter) const
{
    ...
    if(iter==0){
        ...
        size_t aux=universal_hash(k, m_, p_, a_, b_);
...
RHash::operator()(const uint64_t &k, size_t iter) const
{
    ...
    static size_t aux=0;//guarda el ultimo valor que tenia antes de llamarla
    if(iter<a_.size()){//mientras que iter sea menor que el tamanno del vector, el tamanno de a y b es el mismo
        aux=universal_hash(k,m_,p_,a_[iter],b_[iter]);
        ret_v=aux;
    }
    else{
        ret_v=(aux+ (iter-a_.size()) +1)%m_;
        //(iter-a_.size()) lo que se ha pasado iter de size, haces lp de la siguiente posicion, por eso haces +1
    }
    //
    return ret_v;
}
...
DHash::operator()(const uint64_t &k, size_t iter) const
{
    ...
    //        linear probing from the last computed value if it is needed.
       ...
       //los vectores a y b solo tienen dos elementos en este caso
    //
    return ret_v;
}
